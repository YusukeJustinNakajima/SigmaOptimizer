...
function Print-IterationSummary {
    ...
    foreach ($group in $groupedResults) {
        ...
        $candidates = $group.Group | Select-Object CandidateIndex, DetectionResult, Coverage
...
while (($currentIteration -le $maxIterations) -and ($coverage -le 100)) {
    ...
    Print-IterationSummary -Data ($aggregatedCandidateResults | Select-Object Iteration, CandidateIndex, DetectionResult, Coverage)
    Write-Host "`n"
